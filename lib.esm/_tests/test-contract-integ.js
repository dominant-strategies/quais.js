...
describe("Tests contract integration", function () {
    const provider = new quais.JsonRpcProvider("http:/\/127.0.0.1:8545");
    const abi = [
    ...
    it("runs contract operations", async function () {
        ...
        const txInfoData = quais.hexlify(quais.concat([
            "0x6a627842",
            quais.zeroPadValue(await signer.getAddress(), 32)
        ]));
        ...
        assert.ok(receipt instanceof quais.ContractTransactionReceipt, "receipt typeof");
        assert.ok(receipt.logs[0] instanceof quais.EventLog, "receipt.log typeof");
