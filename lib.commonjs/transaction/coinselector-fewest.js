...
class FewestCoinSelector extends abstract_coinselector_js_1.AbstractCoinSelector {
    ...
    performSelection(target) {
        ...
        if (UTXOsEqualOrGreaterThanTarget.length > 0) {
            ...
            while (sortedUTXOs.length > 0 && totalValue < target.value) {
                const nextOptimalUTXO = sortedUTXOs.reduce((closest, utxo) => {
                    ...
                    const absThisDiff = (0, maths_js_1.bigIntAbs)(target.value - (totalValue + utxo.denomination));
                    const currentClosestDiff = closest && closest.denomination
                        ? (0, maths_js_1.bigIntAbs)(target.value - (totalValue + closest.denomination))
                        : BigInt(Infinity);
        ...
        const spendDenominations = (0, utxo_js_1.denominate)(target.value);
        ...
        if (change > BigInt(0)) {
            const changeDenominations = (0, utxo_js_1.denominate)(change);
