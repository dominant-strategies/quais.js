...
function getTransactionHash(tx, data) {
    const destShardbyte = (0, index_js_3.getShardForAddress)(tx.to || "")?.byte.slice(2);
    const destUtxo = (0, index_js_3.isUTXOAddress)(tx.to || "");
    const pubKey = Transaction.from(tx).fromPublicKey;
    const senderAddr = (0, address_js_1.computeAddress)(pubKey || "");
    const originShardByte = (0, index_js_3.getShardForAddress)(senderAddr)?.byte.slice(2);
    const originUtxo = (0, index_js_3.isUTXOAddress)(senderAddr);
    ...
    let hash = (0, index_js_2.keccak256)(data);
