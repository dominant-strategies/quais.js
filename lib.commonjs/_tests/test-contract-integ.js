...
describe("Tests contract integration", function () {
    const provider = new index_js_1.quais.JsonRpcProvider("http:/\/127.0.0.1:8545");
    const abi = [
    ...
    it("deploys a contract", async function () {
        ...
        const factory = new index_js_1.quais.ContractFactory(abi, bytecode, signer);
    ...
    it("runs contract operations", async function () {
        ...
        const contract = new CustomContract(address, signer); //quais.Contract.from<ContractAbi>(address, abi, signer);
        ...
        const txInfoData = index_js_1.quais.hexlify(index_js_1.quais.concat([
            "0x6a627842",
            index_js_1.quais.zeroPadValue(await signer.getAddress(), 32)
        ]));
        ...
        assert_1.default.ok(receipt instanceof index_js_1.quais.ContractTransactionReceipt, "receipt typeof");
        assert_1.default.ok(receipt.logs[0] instanceof index_js_1.quais.EventLog, "receipt.log typeof");
