...
//setupProviders();
const providerC1 = new index_js_1.quais.JsonRpcProvider(process.env.RPC_URL);
const wallet = new index_js_1.quais.Wallet(process.env.FAUCET_PRIVATEKEY || '', providerC1);
...
async function getRPCGasPrice(url) {
    try {
        let response;
        do {
            response = await axios_1.default.post(url || "http://localhost:8610", {
                jsonrpc: "2.0",
...
async function sendTransaction(to) {
    ...
    try {
        ...
        do {
            typeValue = (0, index_js_1.getTxType)(wallet.address, to);
            const gas = await getRPCGasPrice(process.env.RPC_URL);
            let tx = {
                from: wallet.address,
                to,
                value: index_js_1.quais.parseEther("0.1"),
                gasPrice: gas * 2,
                maxFeePerGas: index_js_1.quais.parseUnits('20', 'gwei'),
                maxPriorityFeePerGas: index_js_1.quais.parseUnits('20', 'gwei'),
            ...
            await (0, utils_js_1.stall)(15000);
...
async function fetchRPCBlock(blockNumber) {
    try {
        let response;
        do {
            response = await axios_1.default.post(process.env.RPC_URL || "http://localhost:8610", {
                jsonrpc: "2.0",
...
describe("Test Provider Block operations", function () {
    ...
    it('should fetch block by number', async function () {
        ...
        assert_1.default.ok(responseBlock != null, "block != null");
    ...
    it('should fetch block by hash', async function () {
        assert_1.default.ok(block.hash != null, 'block.hash != null');
