...
class AbstractProvider {
    ...
    async initUrlMap(urls) {
        if (urls instanceof index_js_6.FetchRequest) {
        ...
        for (const url of urls) {
            ...
            this._urlMap.set('0x', primeUrl);
            this.#connect.push(new index_js_6.FetchRequest(primeUrl));
    ...
    shardBytes(shard) {
        return index_js_2.ShardData.find(it => it.name == shard ||
            it.byte == shard ||
            it.nickname == shard ||
            it.shard == shard)?.byte || '';
    }
    ...
    async getLatestQuaiRate(shard, amt = 1) {
    ...
    async getLatestQiRate(shard, amt = 1) {
        const blockNumber = await this.getBlockNumber(shard);
    ...
    async getBlockNumber(shard) {
        const blockNumber = (0, index_js_6.getNumber)(await this.#perform({ method: "getBlockNumber", shard: shard }), "%response");
    ...
    _getBlockTag(shard, blockTag) {
        ...
        if (typeof (blockTag) === "number") {
            ...
            return this.getBlockNumber(shard).then((b) => (0, index_js_6.toQuantity)(b + blockTag));
        }
    ...
    async #getBlock(shard, block, includeTransactions) {
        ...
        if ((0, index_js_6.isHexString)(block, 32)) {
            return await this.#perform({
                method: "getBlock", blockHash: block, includeTransactions, shard: shard
            });
        }
