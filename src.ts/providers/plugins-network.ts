...
export class NetworkPlugin {
     ...
     *
     * It is recommended to use reverse-domain-notation, which permits unique names with a known authority as well as
     * hierarchal entries.
     */
...
/**
 * A **GasCostPlugin** allows a network to provide alternative values when computing the intrinsic gas required for a
 * transaction.
 *
 * @category Providers
 */
export class GasCostPlugin extends NetworkPlugin implements GasCostParameters {
    ...
    /**
     * Creates a new GasCostPlugin from `effectiveBlock` until the latest block or another GasCostPlugin supercedes that
     * block number, with the associated `costs`.
     */
 ...
 *
 * For example, a network which does not support [EIP-1559](https://eips.ethereum.org/EIPS/eip-1559) may choose to use a
 * Gas Station site to approximate the gas price.
 *
 * @category Providers
 */
...
export class FetchUrlFeeDataNetworkPlugin extends NetworkPlugin {
    ...
    readonly #processFunc: (
        ...
        p: Provider,
        r: FetchRequest,
    ) => Promise<{ gasPrice?: null | bigint; maxFeePerGas?: null | bigint; maxPriorityFeePerGas?: null | bigint }>;
