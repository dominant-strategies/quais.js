...
export class QuaiHDWallet extends BaseWallet {
     ...
     * ({@link HDNodeVoidWallet | **HDNodeVoidWallet**}) or full HD Node ({@link QuaiHDWallet | **QuaiHDWallet**})
     * respectively.
     *
    ...
    deriveAddress(index: number, zone?: string): QuaiHDWallet {
        ...
        do {
            ...
            if (
                getShardForAddress(newWallet.address) == shard &&
                (newWallet.coinType == 969) == isUTXOAddress(newWallet.address)
            )
                zoneIndex--;
            addrIndex++;
        } while (zoneIndex > 0);
 ...
 * {@link QuaiHDWallet.fromExtendedKey | **QuaiHDWallet.fromExtendedKey**} or by [nuetering](QuaiHDWallet-neuter) a
 * {@link QuaiHDWallet | **QuaiHDWallet**}.
 *
 * @category Wallet
 */
