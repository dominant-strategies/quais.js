...
import {
	...
	isUTXOAddress, BytesLike, Numeric, defineProperties, 
	getBytes, dataSlice,
} from "../utils/index.js";
...
/**
 *  An **HDWallet** is a [Signer](../interfaces/Signer) backed by the private key derived
...
export abstract class HDWallet extends BaseWallet implements HDNodeLike<HDWallet> {
	...
	protected static _fromSeed<T extends HDWallet>(this: HDWalletStatic<T>,_seed: BytesLike,mnemonic: null | Mnemonic): T {
	 ...
	 *  or full HD Node ({@link HDWallet | **HDWallet**}) respectively.
	 *
	 ...
	 */
	static fromExtendedKey<T extends HDWallet>(this: new (...args: any[]) => T,extendedKey: string): T | HDNodeVoidWallet {
	 ...
	 */
	static createRandom<T extends HDWallet>(this: HDWalletStatic<T>, path: string, password?: string, wordlist?: Wordlist): T {
	 ...
	 *  @returns {HDWallet} The new HD Node Wallet.
	 */
	static fromPhrase<T extends HDWallet>(this: HDWalletStatic<T>, phrase: string, path: string, password?: string, wordlist?: Wordlist): T {
		if (path == null || !this.isValidPath(path)) {
			throw new Error("Invalid path: " + path);
		}
 ...
 *  {@link HDWallet.fromExtendedKey | **HDWallet.fromExtendedKey**} or by
 *  [nuetering](HDWallet-neuter) a {@link HDWallet | **HDWallet**}.
 *
 *  @category Wallet
 */
export class HDNodeVoidWallet extends VoidSigner {
	 ...
	 *  @private
	 */
	constructor(guard: any,address: string,_publicKey: string,accountFingerprint: string,chainCode: string,path: null | string,index: number,depth: number,provider: null | Provider) {
 ...
 *  @category Wallet
 */
export function qiHDAccountPath(
	account: number = 0,
